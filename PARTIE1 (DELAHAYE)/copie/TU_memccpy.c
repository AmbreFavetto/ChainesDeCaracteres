...
int main (void) {
    ...
    /* chaîne de caractères pour tester les chaînes non initialisées */
    char chaine[10];

    /* retour de la fonction */
    char * p_out;

    char * sup = "LES GUITARES";
    char * egal = "!GUITARES";
    char * inf = "GUITARE";
    ...
    for (i = 0; i < TAILLE; i++) {
        ...
        p_out = memccpy(s_out, p->in.source, p->in.carac, p->in.aCopier);
        *p_out = p_out[strlen(s_out) - 1];

        /* verifications */
        if (*p_out != 'S') {
            printf ("error at test # %d (wrong returned address) : %c\n", p->n,*p_out);
            err = 1;
        }
